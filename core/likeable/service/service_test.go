...
func TestLikeChecker(t *testing.T) {
	targetId := RandomId()
	userId := RandomId()
	t.Run("happy case", func(t *testing.T) {
		isLiked := RandomBool()
		storeChecker := func(id string, fromUser core_values.UserId) (bool, error) {
			if id == targetId && fromUser == userId {
				return isLiked, nil
			}
			panic("unexpected args")
		}
		got, err := service.NewLikeChecker(storeChecker)(targetId, userId)
		AssertNoError(t, err)
		Assert(t, got, isLiked, "isLiked")
	})
	t.Run("error case - store throws", func(t *testing.T) {
		storeChecker := func(string, core_values.UserId) (bool, error) {
			return false, RandomError()
		}
		_, err := service.NewLikeChecker(storeChecker)(targetId, userId)
		AssertSomeError(t, err)
	})
}

func TestLikesCountGetter(t *testing.T) {
	targetId := RandomId()
	t.Run("happy case", func(t *testing.T) {
		likes := RandomInt()
		storeLikesGetter := func(id string) (int, error) {
			if id == targetId {
				return likes, nil
			}
			panic("unexpected args")
		}
		gotLikes, err := service.NewLikesCountGetter(storeLikesGetter)(targetId)
		AssertNoError(t, err)
		Assert(t, gotLikes, likes, "returned likes count")
	})
	t.Run("error case", func(t *testing.T) {
		storeLikesGetter := func(string) (int, error) {
			return 0, RandomError()
		}
		_, err := service.NewLikesCountGetter(storeLikesGetter)(targetId)
		AssertSomeError(t, err)
	})
}
